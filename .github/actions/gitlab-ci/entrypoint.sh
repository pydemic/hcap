...
pipeline_id=$(curl --header "PRIVATE-TOKEN: $GITLAB_PASSWORD" --silent "https://gitlab.com/api/v4/projects/18009514/repository/commits/${branch}" | jq '.last_pipeline.id')
...
do
   sleep $POLL_TIMEOUT
   ci_output=$(curl --header "PRIVATE-TOKEN: $GITLAB_PASSWORD" --silent "https://gitlab.com/api/v4/projects/18009514/pipelines/${pipeline_id}")
